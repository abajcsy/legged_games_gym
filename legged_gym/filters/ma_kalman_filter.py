...
class MultiAgentKalmanFilter(object):
    def __init__(self, 
                    dt, 
                    num_states, 
                    num_actions_r, 
        ...
        # 
        #   state = (x_rel, y_rel, z_rel, dtheta) 
        # 
        # where dtheta ~= (atan(y_rel, x_rel) - theta_r) and 
        # 
        #   robot control = (vx_r, vy_r, omega_r)
        #   agent control = (vx_a, vy_a, omega_a)
        # 
        ...
        self.B_r = -self.dt * torch.eye(self.num_states, self.num_actions_r, dtype=self.dtype, device=self.device)
        self.B_a = self.dt * torch.eye(self.num_states, self.num_actions_a, dtype=self.dtype, device=self.device)
        ...
        self.B_r_tensor = self.B_r.repeat(self.num_envs, 1).reshape(self.num_envs, self.num_states, self.num_actions_r)
        self.B_a_tensor = self.B_a.repeat(self.num_envs, 1).reshape(self.num_envs, self.num_states, self.num_actions_a)
    ...
    def dynamics(self, x, command_robot, command_agent):
        """
        Applies linear dynamics to evolve state: x' = Ax + B_r * u_r + B_a * u_a

        Args: 
            x (torch.Tensor): current state of shape [num_envs, num_states]
            command_robot (torch.Tensor): current action of shape [num_envs, num_actions_r]
            command_agent (torch.Tensor): current action of shape [num_envs, num_actions_a]
        Returns:
        ...
        if command_agent.dtype != self.dtype:
            command_agent = command_agent.to(self.dtype)

        Ax = torch.bmm(self.A_tensor, x.unsqueeze(-1)).squeeze(-1)
        Brur = torch.bmm(self.B_r_tensor, command_robot.unsqueeze(-1)).squeeze(-1)
        Baua = torch.bmm(self.B_a_tensor, command_agent.unsqueeze(-1)).squeeze(-1)
        xnext = Ax + Brur + Baua
    ...
    def predict(self, command_robot, command_agent):
        """
        ...
        #   xhat' = Axhat + Br * ur + Ba * ua
        self.xhat = self.dynamics(self.xhat, command_robot, command_agent)
    ...
    def _evade_command_agent(self, rel_state):
        command_agent = torch.zeros(self.num_envs, self.num_actions_a, device=self.device, requires_grad=False)
        rel_state_pov_agent = -rel_state
        for env_idx in range(self.num_envs):
            command_agent[env_idx, :2] = torch.clip(-rel_state_pov_agent[env_idx, :2], min=-1, max=1)
...
def simulate():
    ...
    for tidx in range(len(t)):
        action_r = kf._turn_and_pursue_command_robot(xrel)
        action_a = kf._evade_command_agent(xrel)
        xrel = kf.dynamics(xrel, action_r, action_a)
    ...
    for tidx in range(len(t)):
        print("predicting...")
        action_r = kf._turn_and_pursue_command_robot(kf.xhat)
        action_a = kf._evade_command_agent(real_traj[tidx, :]) # TODO: agent gets ground-truth state
        xhat = kf.predict(action_r, action_a)
